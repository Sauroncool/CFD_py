...
import numpy as np
from single_interpolation import parametric_interpolation,load_airfoil_data  # Import airfoil generation

def grid_generation(num_xi, num_eta, R_outer):
    ...
    for i in range(num_xi):
        for j in range(num_eta):
            x[i, j] = (1 - eta[j]) * x_inner[i] + eta[j] * R_outer * np.cos(2 * np.pi * xi[i])
            y[i, j] = (1 - eta[j]) * y_inner[i] + eta[j] * R_outer * np.sin(2 * np.pi * xi[i])
...
if __name__ == "__main__":
    ...
    # Generate the grid
    x, y = grid_generation(num_xi, num_eta, R_outer)
